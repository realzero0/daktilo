...
쓰레드 스케쥴러는 어떤 쓰레드를 얼마나 오랫동안 실행할지를 결정한다.

운영체제는 쓰레드를 선택하는 결정을 공평하게 하려고 하지만, 적용 정책은 각각 다르다.

잘 작성된 프로그램은 이런 정책의 디테일에 의존해서는 안된다.

어떤 프로그램이든 정확도(correctness)의 이유든 성능의 이유든 쓰레드 스케쥴러에 의존하는 어떤 프로그램도 여러 운영체제에 사용할 수 없다.

<br>
<br>

실행가능한(Runnable) 쓰레드의 갯수가 현재 CPU 프로세서 수에 비해서 너무 많지 않으면 강하고, 반응성이 뛰어나고, 다른 운영체제에 이식 가능한 프로그램을 작성할 수 있다.

이렇게 되면 쓰레드 스케쥴러는 사실 하는일이 많지 않다. 실행가능한 쓰레드를 실행불가능할 때까지 실행하는 게 전부다.

쓰레드 스케쥴링이 아주 많이 차이가 나더라도 프로그램의 동작은 별로 바뀌지 않게 된다.

실행가능한 쓰레드의 갯수와 전체 쓰레드 갯수는 다르다. Waiting 상태의 쓰레드는 실행가능하지 않다.

<br>
<br>

runnable 쓰레드의 갯수를 적게 유지하는 방법
- 쓰레드가 필요한 작업시에만 동작하고 그렇지 않으면 wait 상태가 되어야 한다.
    - Executor Framework에서는 
        - 쓰레드 풀의 사이즈를 적절히 조절해야한다.
        - task를 적당히 짧게 유지해야한다. (너무 짧으면 dispatching overhead가 생길 수 있다)

<br>
<br>

- 쓰레드는 busy-wait 상태이면 안된다.
    - busy-wait이면 공유된 객체의 상태가 변화하기를 계속해서 체크한다.
    - 쓰레드 스케쥴러의 변화에 프로그램이 취약해진다.(쓰레드 스케쥴러가 운영체제에 의해 변경되면 매우 느려질 수 있다)
    - busy-wait은 프로세서의 load를 상당히 높이게 된다. (이는 꼭 필요한 작업이 실행되지 못하게 한다)
...
저자의 컴퓨터에서는 1000개의 쓰레드가 기다리는 경우에 10배 정도 느리다고 한다.

이 예제가 극단적이기는 하지만, 하나 이상의 쓰레드가 불필요하게 runnable인 경우는 많다.
...
특정 쓰레드가 충분한 CPU time을 획득하지 못해서 잘 동작하지 못하는 프로그램이 있다면, `Thread.yield`를 사용하려고 하면 안된다.

이식성이 매우 떨어지게 되고, 적절히 테스트할 방법이 없다.

병렬적으로 처리되는 runnable 쓰레드를 줄이는게 낫다.


<br>
<br>

쓰레드의 우선순위를 조정하는 방법이 있다. 자바 기능 중에서 가장 작은 이식성을 가진 기능이다.

그렇지만 어플리케이션의 반응성을 향상시키기 위해서 쓰레드 우선순위를 조정한다는 것은 그렇게 말이 되지 않고, 아주 가끔 필요할 것이다.

심각한 liveness 문제를 쓰레드 우선순위를 조절해서 해결하려고 하는 것은 사실 말이 안된다. 실제 원인은 다른 곳에 있을 수 있다.
...
프로그램이 쓰레드 스케쥴러에 의존하게 하지말자. 의존하게 되면 프로그램은 깨지기 쉬워지고 이식성도 떨어지게 된다.
